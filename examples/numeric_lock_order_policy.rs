...
use syn::{Expr, ExprCall, ImplItem, Item, Local, Pat};
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let bindings = collect_lock_bindings(&locals, &exact_name_ranks);
    let acquisitions = collect_acquisitions(&locals, &bindings);
...
fn collect_locals(function: &syn::ItemFn) -> Vec<Local> {
        ...
        Expr::Call(call) => {
                ...
                .is_some_and(|name| matches!(name.as_str(), "new" | "with_name"))
            {
                for argument in &call.args {
                    if let Some(name) = literal_string(argument) {
                        return Some(name);
                    }
                }
            }
...
fn print_boundary() {
    ...
    println!(
        "  The source adapter recursively visits local bindings inside the selected function, including nested blocks and closures, and recognizes literal lock names plus named `.lock()` acquisitions."
    );
    println!(
        "  The lexical traversal is control-flow-insensitive and does not model releases. Mutually exclusive branches, async lock futures, aliases, dynamic names, and prefix-derived ranks remain outside this probe."
    );
}
...
mod tests {
    ...
    fn extracts_nested_literal_lock_constructors() {
        let function = find_function(
            ...
            fn example() {
                run(|| {
                    let tasks_lock = Arc::new(ContendedMutex::new(0, "tasks_queue"));
        ...
        let ranks = parse_exact_name_ranks(INVENTORY).unwrap();
        let locals = collect_locals(&function);
        let bindings = collect_lock_bindings(&locals, &ranks);
